CREATE OR REPLACE PACKAGE PACKAGE_STATS_FUNCTION AS
...
END;
/
CREATE OR REPLACE PACKAGE BODY PACKAGE_STATS_FUNCTION AS
    ...
    FUNCTION nr_posts_by_category return sys_refcursor AS
        v_cursor sys_refcursor;
    BEGIN
        open v_cursor for select c.name, count(*) from category c join thread th on c.id = th.category_id group by c.name, c.id;
        return v_cursor;
    END;
