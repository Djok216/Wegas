...
CREATE OR REPLACE PACKAGE PACKAGE_USERS AS
  ...
  PROCEDURE INSERT_NEW_REGULAR_USER_FB(p_facebookId USERS.FACEBOOK_ID%TYPE, p_email USERS.EMAIL%TYPE, p_nickname USERS.NICKNAME%TYPE, p_name USERS.NAME%TYPE);
...
CREATE OR REPLACE PACKAGE BODY PACKAGE_USERS AS
  ...
  PROCEDURE INSERT_NEW_REGULAR_USER_FB(p_facebookId USERS.FACEBOOK_ID%TYPE, p_email USERS.EMAIL%TYPE, p_nickname USERS.NICKNAME%TYPE, p_name USERS.NAME%TYPE) AS
    v_count INTEGER;
  BEGIN
    -- 2 for regular user
