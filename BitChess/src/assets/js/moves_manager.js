function make_moves_el(parent, height_parent) {
  ...
  function add_move(options) {
    ...
    plys[options.ply] = {
      ...
      san_el: G.cde("div", {
        c: "moveCell moveSAN move" + options.color + " moveRow" + even_odd + clickable_cell,
        t: clean_san(options.san)
      }, {click: options.onclick}),
      eval_el: G.cde("div", {
        c: "moveCell moveEval move" + options.color + " moveRow" + even_odd + clickable_cell,
        t: "\u00a0"
      }, {click: options.onclick}), /// \u00a0 is &nbsp;
      time_el: G.cde("div", {
        c: "moveCell moveTime move" + options.color + " moveRow" + even_odd + clickable_cell,
        t: typeof options.time === "number" ? format_move_time(options.time) : "\u00a0"
      }, {click: options.onclick}),
    };
    ...
    plys.forEach(function oneach(move_data, ply) {
      ...
      function add_placeholding_els() {
        ...
        for (i = 0; i < len; i += 1) {
          ...
          placeholders[i] = G.cde("div", {
            c: "moveCell moveSAN move" + (color === "w" ? "b" : "w") + " moveRow" + even_odd,
      ...
      if (!rows[cur_row]) {
        ...
        rows[cur_row].row_el.appendChild(G.cde("div", {
          c: "moveNumCell moveRow" + even_odd + clickable_cell,
          t: (cur_row + 1)
        }, num_events));
